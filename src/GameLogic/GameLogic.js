...
class GameLogicEngine {
    ...
    rollDicePlayer = () => {
        ...
        this.graphicEngine.fonts(`Agora e a vez do ${player} jogar o dado`,
            this.configsEngine.colorsToPlayers[player], () => this.rollDiceButton(this.configsEngine.colorsToPlayers[player]), () => this.cameraMove(player))
    }

    playersStart = () => {
        const player = this.configsEngine.players[this.configsEngine.currentPlayerIndex];
        this.graphicEngine.fonts(`
                    Ordem de Jogada: 
                    1: ${this.configsEngine.players[0]} 
                    2: ${this.configsEngine.players[1]} 
                    3: ${this.configsEngine.players[2]} 
                    4: ${this.configsEngine.players[3]}`,
            this.configsEngine.colorsToPlayers[player], () => { });
        this.rollDicePlayer();
    };
    ...
    chosePinText = () => {
        ...
        this.graphicEngine.fonts(`${player} vai se mover, escolha o pino.`,
            this.configsEngine.colorsToPlayers[player], () => this.pinsButtons(this.configsEngine.colorsToPlayers[player], 'active', 'invisible'), () => this.cameraMove(player, () => this.cameraMove('Default')))
    }

    selectedPinText = (pino) => {
        const player = this.configsEngine.players[this.configsEngine.currentPlayerIndex];
        this.graphicEngine.fonts(`Pino Selecionado: ${pino}.`, this.configsEngine.colorsToPlayers[player], this.pinsButtons(this.configsEngine.colorsToPlayers[player], 'invisible', 'active'))
        this.graphicEngine.fonts(`Cor ${player} pode jogar o dado novamente.`, this.configsEngine.colorsToPlayers[player], () => this.rollDiceButton(this.configsEngine.colorsToPlayers[player]));
    }
    ...
    aguardarEscolhaPino = () => {
        ...
        this.graphicEngine.fonts(`${player} escolha um pino para se mover.`,
            this.configsEngine.colorsToPlayers[player], player, () => this.pinsButtons(this.configsEngine.colorsToPlayers[player], 'active', 'invisible'))
    ...
    esperandoEscolhaPino = (numeroPino, topFaceIndex) => {
        ...
        if (this.configsEngine.esperandoEscolhaMoverPinoAtual) {
            this.movePinSteps(topFaceIndex, 0, () => {
                ...
                e o ${pinoSelecionado} voltara para o Inicio`, this.configsEngine.colorsToPlayers[player], () => { });
                    this.graphicEngine.fonts(`Cor ${player} pode jogar o dado novamente.`, this.configsEngine.colorsToPlayers[player], player, () => this.rollDiceButton(this.configsEngine.colorsToPlayers[player]), () => cameraMove(player));
    ...
    newPinFunc = () => {
        ...
        this.graphicEngine.fonts(`O ${player} escolheu adicionar um novo pino`,
            this.configsEngine.colorsToPlayers[player], () => { }, () => this.cameraMove(player))
        ...
        if (this.configsEngine.esperandoDecisaoUsuario) {
            ...
            this.movePinSteps(this.configsEngine.topFaceIndex, 0, () => {
                ...
                if (sobreposicao.encontrou) {
                    ...
                    const pino = this.configsEngine.pinos[pinoSobreposto];
                    this.graphicEngine.fonts(`Cor ${sobreposicao.detalhes[0].corOriginal} sobrepos a Cor ${sobreposicao.detalhes[0].corSobreposta}, 
                e o ${pinoSelecionado} voltara para o Inicio`, this.configsEngine.colorsToPlayers[corAtualJogador], () => { });
                    this.graphicEngine.fonts(`Cor ${corAtualJogador} pode jogar o dado novamente.`, this.configsEngine.colorsToPlayers[corAtualJogador], () => this.rollDiceButton(this.configsEngine.colorsToPlayers[corAtualJogador]), () => this.cameraMove(corAtualJogador));
                    let novaEtapa; // variável para a nova etapa
            ...
            this.newPin.classList.remove('active');
            this.configsEngine.esperandoDecisaoUsuario = false;
            this.graphicEngine.fonts(`${corAtualJogador} escolheu mover o pino atual, ${this.configsEngine.topFaceIndex} casas`, this.configsEngine.colorsToPlayers[corAtualJogador], () => { }, () => this.cameraMove(corAtualJogador))
            this.graphicEngine.fonts(`Cor ${corAtualJogador} pode jogar o dado novamente.`, this.configsEngine.colorsToPlayers[corAtualJogador], corAtualJogador, () => this.rollDiceButton(this.configsEngine.colorsToPlayers[corAtualJogador]));
        } else {
                ...
                if (pinoParaMover !== null) {
                    this.movePinSteps(topFaceIndex, 0, () => {
                        ...
                        if (sobreposicao.encontrou) {
                            ...
                            this.graphicEngine.fonts(`Cor ${sobreposicao.detalhes[0].corOriginal} sobrepos a Cor ${sobreposicao.detalhes[0].corSobreposta}, 
                e o ${pinoSelecionado} voltara para o Inicio`, this.configsEngine.colorsToPlayers[corAtualJogador], () => { });
                            fonts(`Cor ${corAtualJogador} pode jogar o dado novamente.`, this.configsEngine.colorsToPlayers[corAtualJogador], () => this.rollDiceButton(this.configsEngine.colorsToPlayers[corAtualJogador]), () => this.cameraMove(corAtualJogador));
                            let novaEtapa; // variável para a nova etapa
