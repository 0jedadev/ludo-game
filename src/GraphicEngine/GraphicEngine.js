...
class GraphicEngine {
    constructor(configs) {
        ...
        this.renderer.setClearColor('#9b9b9b');
    ...
    Floor = () => {
        ...
        const belowFloorMaterials = [
            new THREE.MeshStandardMaterial({ map: this.colVar1Texture }), // Material para o lado esquerdo
            new THREE.MeshStandardMaterial({ map: this.colVar2Texture }), // Material para o lado direito
            new THREE.MeshStandardMaterial({ map: this.aoTexture }), // Material para o fundo
            new THREE.MeshStandardMaterial({ map: this.colVar3Texture }), // Material para o frente
            new THREE.MeshStandardMaterial({
    ...
    displayNextText = () => {
        if (this.configs.textQueue.length > 0 && !this.configs.isTextBeingDisplayed) {
            ...
            const { textFont, color, callback, secondCallback } = this.configs.textQueue[0]; // Pega o próximo texto da fila, mas não remove ainda
            this.fontLoader.load(
                ...
                (font) => {
                    ...
                    const material = new THREE.MeshMatcapMaterial({ color: color, matcap: this.matcapTexture });

                    const newTextObject = new THREE.Mesh(textGeometry, material);

                    // Agora que o novo Mesh foi criado, você pode ajustar sua posição
                    newTextObject.position.set(0, 2, 0);

                    if (this.configs.textObject) {
                        this.scene.remove(this.configs.textObject);
                    }
                    this.configs.textObject = newTextObject;
                    this.scene.add(this.configs.textObject);
                    // Aguarda um tempo antes de exibir o próximo texto
                    setTimeout(() => {
                        // Remove o texto da cena após o tempo determinado
                        if (this.configs.textObject) {
                            this.scene.remove(this.configs.textObject);
                        }
                        // Remove o texto da fila
                        this.configs.textQueue.shift();
                        // Define que não há mais texto sendo exibido
                        this.configs.isTextBeingDisplayed = false;
                        // Se houver um callback, execute-o
                        ...
                        // Chama a função novamente para verificar se há mais textos na fila
    ...
    fonts = (textFont, color, callback, secondCallback) => {
        console.log(textFont, color);
        // Adiciona o novo texto à fila
        this.configs.textQueue.push({ textFont, color, callback, secondCallback });
